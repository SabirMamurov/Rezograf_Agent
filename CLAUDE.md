...
**The `Печать` button MUST send the image bitmap to the printer, never the PDF.** `handlePrint` in `src/app/print/page.tsx` calls `/api/render` with `format: "image"`, embeds the returned PNG in a hidden iframe sized exactly `wMm × hMm` with `image-rendering: pixelated`, and triggers `iframe.contentWindow.print()`. This way the printer driver receives a bitmap pre-thresholded to the head's native 203 DPI and just stamps it pixel-for-pixel. The PDF path was tried once (same `64c8985`) "because it's 3× faster" — the printer driver then re-rasterized the PDF on its own, antialiased the body text, and produced exactly the same wavy / dithered output that the image path was designed to prevent. Operators reported it twice. PDF download (`handleRenderPdf`, the second button) is fine because the user opens it in a viewer; only the *direct print path* must be the bitmap.
